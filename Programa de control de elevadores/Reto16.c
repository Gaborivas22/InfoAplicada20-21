...
int nivelOrigen, nivelDestino, numeroNiveles, contPersonas, totalPersonas, nivelActual, elevadores[5]= {0,0,0,0,0}, idElevador;
...
int main() { 
	...
	while (toupper(finPrograma)== 'S'){
		...
		if(nivelActual>-1){
			...
			while (elevadores[idElevador-1]!=nivelOrigen){		
				if(elevadores[idElevador-1]<nivelOrigen) {
					elevadores[idElevador-1]++;
